...
description: 当用户说“双语化”“双语化 skill”“双语化这个 skill”“汉化 skill”“本地化 skill”或“创建双语 skill”时，使用此技能翻译和本地化 Codex 技能；负责双语名称与简介、五种本地化版本、中文 slash 入口、审批和最终验证。 When the user asks to bilingualize a skill, Chinese-localize a skill, or create a bilingual skill, use this skill to localize Codex skills with bilingual metadata, five UI variants, slash aliases, approval, and verification.
---
...
明确目标后，在同一条对话消息中展示拟执行方案和以下五个版本，供用户直接选择：
...
5. **中文标题（英文原名）+ 中文简介（Chinese title + English original in full-width parentheses + Chinese description）**：`display_name` 使用中文标题，并使用全角括号 `（英文原名）` 逐字保留英文原名；`short_description` 完整使用中文简介；触发用的 frontmatter `description` 仍默认双语。此版本与版本 2 的语义相同，但固定使用中文全角括号，作为独立版本供用户明确选择。

同一条助手回复中还必须列出：默认版本为版本 5；目标技能、创建位置或安装位置、将修改的文件、五种版本分别对应的 `display_name`、`short_description` 和 frontmatter `description`、slash 入口、是否创建软链接、空格与大小写处理，以及关键差异示例。

除版本字段外，确认前的方案必须根据目标 skill 的实际能力提供简短具体示例。每个版本至少展示一个与目标 skill 相关的示例，优先包括：拟生成的标题、拟生成的简介、一个可触发该 skill 的用户说法，或一个中文 slash 调用示例。例如，目标是 ATS 简历优化 skill 时，应展示“帮我检查这份简历的 ATS 关键词匹配度”这类真实示例；不得只写“示例同上”“根据目标 skill 生成”或其他泛化占位内容。此阶段只读，不得写入文件。等待用户明确选择版本并批准当前完整方案；未指定版本时默认采用版本 5；只有“选择版本 5，并批准按上面方案执行”这类明确绑定方案的回复才算批准。“看一下”“怎么样”“先给我方案”以及只说“版本 5”都不算批准。

### 第三步：执行（Apply）

只有用户选择版本并明确批准当前完整方案后，才进行写入或创建操作。删除旧技能、覆盖已有文件、替换已有软链接、改动原英文入口等破坏性操作，必须在方案中单独标注，并获得明确授权；不能从普通的“批准方案”中推断出删除授权。执行删除、覆盖或替换前，先运行 `python3 scripts/skill_backup.py create <真实技能目录> --label <说明>`，记录备份路径并检查备份清单。若需要回滚，先向用户说明恢复目标，再运行 `python3 scripts/skill_backup.py restore <备份目录或 ID> <目标目录> --replace`；恢复前脚本会再次创建 `pre-restore` 备份。若用户要求改动方案，必须回到第二步重新展示五个版本并请求批准。
...
- 运行结构化验证器：`python3 scripts/validate_bilingual_skill.py <真实技能目录> --version <1|2|3|4|5> --alias <中文入口路径> [--require-translation]`。
...
- 第 1 版本为中文标题 + 中文简介；第 2 版本在 UI `display_name` 中保留英文原名；第 3 版本在 UI `short_description` 中使用英文简介；第 4 版本使用英文标题 + 中文简介；第 5 版本在 UI `display_name` 中使用中文标题和全角括号包裹的英文原名，并使用中文简介。
...
- 同一条助手回复必须同时包含五个版本，不要拆成多轮分别展示，以免用户在不同版本之间批准了不一致的方案。
- 五个版本只改变用户可见的 UI 字段；内部 `name`、触发用 `description` 和技术路径默认保持稳定，除非用户明确批准改变。
